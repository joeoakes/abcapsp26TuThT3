...
## What Happens Step by Step When the Maze App Sends Mission Data

1. The maze app initiates a connection to `https://localhost:8445/mission`.
...
8. The maze app sends mission-summary JSON (`mission_id`, move counters, result, etc.).
9. The server stores it in Redis as `team3ttmission:<mission_id>` and responds with `{"ok":true,...}`.
